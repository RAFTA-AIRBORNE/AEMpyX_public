...
"""
InFileFmt = ".npz"
InModDir = AEMPYX_DATA + "/Projects/Compare/results/LType/"
InModDir = AEMPYX_DATA + "/Projects/StGormans/results/"
...
PlotDir = "/home/vrath/AEM_Plots/AEM05/StGormans/"
...
PdfCatalog = True
if ".pdf" in PlotFmt:
    PdfCatName = PlotDir+"AEM05_Type"+str(PlotType)+"StGormans_Catalog.pdf"
else:
...
if max_doi:    
    alpha_doi = 0.00
    if "man" in  meth_doi.lower():
        maxdoi = 150.
    if "old" in  meth_doi.lower():
        strng_doi = "Li & Oldenburg 1999"
        doithresh = 0.2
    if "sens" in  meth_doi.lower():
        strng_doi = "Li & Oldenburg 1999"
        lowsens = -2.
        # sens_pars = ["euc","size", "sqrt","max", "log"]
        sens_pars = ["euc","sqrt","max", "log"]
...
for file in res_files:
    ...
    if PlotType in [0, 1, 2]:
        ...
        cb = fig.colorbar(matplotlib.cm.ScalarMappable(norm=norm, cmap=mycmap), cax=axii,
                          ticks=cb_ticks ,
