...
def run_jcn(Ctrl=None, Model=None, Data=None, OutInfo=False):
    ...
    jcn_var = fac * numpy.sum((m_jcn - numpy.tile(jcn_avg, (1, jcn_num))**2))
...
def run_rto(Ctrl=None, Model=None, Data=None, OutInfo=False):
    ...
    """
    unpack contol variables and run reference model
   ...
   run reference model
    ...
    rto_mad = numpy.median(numpy.abs(rto_ens - rto_med))
...
# @ray.remote
def run_ekinv(Ctrl=None, Model=None, Data=None, OutInfo=True):
    """
    Ensemble Kalman inversion
    M. A. Iglesias, K. J. H. Law, and A. M. Stuart,
        “The ensemble Kalman filter for inverse problems”
        Inverse Problems, vol. 29, Art. no. 4, 2013,
    ...
    C.-H. M. Tso, M. Iglesias, P. Wilkinson, O. Kuras, J. Chambers, A. Binley
        ...
        Geophysical Journal International, vol. 225, Art. no. 2, 2021
    ...
    unpack control block:
        ...
        Ctrl_EKinv= dict([("aemsys", AEM_system),
                    ("system", [AEM_system, FwdCall]),
                    ("transform", [ParaTrans, DataTrans]),
    ...
    """
    m_ens = inverse.generate_model_ensemble(Mref=m_ref,
                                            Nens=nsamples,
                                            Perturb=["Gauss" ,Cm1],
                                            InChol = numpy.array([]),
                                            InCovar = Cm1,
    ...
    for niter in numpy.arange(maxiter):
        ...
        for isample in numpy.arange (nsamples):
            ...
            d_ens[isample,:],_ = inverse.calc_fwdmodel(
                fwdcall=fwdcall,
                alt=alt,
                m_vec=m,
                m_trn=m_trn,
        ...
        Gyy = inverse.calc_ecovar(x=d_ens, y=d_ens, method = 0, OutInfo=True)
        Gxy = inverse.calc_ecovar(x=m_ens, y=d_ens, method = 0, OutInfo=True)
