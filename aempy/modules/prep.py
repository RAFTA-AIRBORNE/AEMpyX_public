...
def handle_gaps(M, columns=[0, 0], Impute=['delete'], System = 'aem05'):
    ...
    if 'nan' in method[0].lower():
        ...
        sys.exit('Method ' + method + ' not yet implemented! Nothing done.')
...
def calc_svd_decomp(D=None, columns=None, ErrD=None, k=1, thresh_MSE=0.,
    ...
    if (D is None) or (columns is None):
        sys.exit('pcafilter: no data definded! ')
...
#     else:
#         sys.exit('method ' + meth.lower() + ' not implemented! Exit.')
...
def process_column(datavec=numpy.array([]),
    ...
    if 'mean' in Method[0].lower():
        ...
        if ErrEst:
            sys.exit('For method ' + Method[0].lower() + ' no error esimation possible! Exit.')
    else:
        sys.exit('method ' + Method[0].lower() + ' not implemented! Exit.')
