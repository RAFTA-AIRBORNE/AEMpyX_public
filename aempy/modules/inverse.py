...
def run_inv_flightline(data_dir= None,
    ...
    if data_file is None:
        sys.exit('run_inv: No data_dict file given! Exit.')
    ...
    if 'read' in setprior:
        if name.lower() not in prior_file.lower():
            sys.exit('Halfspace file name does not match! Exit.')
...
# This is the main loop over sites in a flight line or within an area:
    ...
    # for ii in [0, 1, 2]:
        ...
        if ('tikh' in runtype) and ('opt' in runtype):
            ...
            sys.exit('run_inv_flightline: runtype '+runtype+' not implementd! Exit.')
...
def run_tikh_opt(Ctrl=None, Model=None, Data=None, OutInfo=False):
    ...
    while (niter < maxiter):
        ...
        for t0 in tau0:
            ...
            for t1 in tau1:
                ...
                if 'ufc' in regfun.lower():
                    ...
                    sys.exit('Regularisation method '+regfun.lower() +
        ...
        if 'fix' in regfun.lower():
            ...
            sys.exit(regfun.lower() + ' not implemented!')
        # print(reg_choice)
...
'''
    ...
    while (niter < maxiter):
        ...
        if 'fix' in regfun.lower():
            ...
            sys.exit(regfun.lower() + ' not iplemented!')
...
def run_most_squares(OutInfo=True):
    ...
    '''
    msqresults = dict([])
    sys.exit('This algorithm is not yet implemented! Exit.')
    return msqresults
...
def calc_fwdmodel(fwdcall=None,
    ...
    if numpy.size(m_vec) == 0:
        sys.exit('calc_fwdmodel: No model given! Exit.')
...
def perturb_random(vbase=numpy.array([]),
    ...
    '''
    if vbase.size == 0:
        sys.exit('generate_ensemble: No Reference model given! Exit.')
...
def generate_data_ensemble(dref=numpy.array([]),
    ...
    '''
    if dref.size == 0:
        sys.exit('generate_data_ensemble: No Reference model given! Exit.')

    ndat = numpy.shape(dref[dact != 0])[0]
    if ndat == 0:
        sys.exit('generate_data_ensemble: no active data given! exit.')
...
def generate_param_ensemble(mref=numpy.array([]),
    ...
    '''
    if mref.size == 0:
        sys.exit('generate_param_ensemble: no reference model given! exit.')

    mpar = numpy.shape(mref[mact != 0])[0]
    if mpar == 0:
        sys.exit('generate_param_ensemble: no active parameter given! exit.')
...
def calc_encovar(x=numpy.array([]),
    ...
    if (x.size == 0) and (y.size == 0):
        sys.exit('calc_encovar: No data given! Exit.')
...
def calc_keg_update(m=numpy.array([]), r=numpy.array([]),
    ...
    '''
    if (m.size == 0):
        sys.exit('calc_kupdate: No model given! Exit.')
    if (r.size == 0):
        sys.exit('calc_kupdate: No residuals given! Exit.')
    ...
    if (Cd.size == 0):
        sys.exit('calc_keg_update: Data covariance missing! Exit.')
    else:
...
def get_nlyr(model_vec=numpy.array([])):
    ...
    if numpy.size(model_vec) == 0 or numpy.size(model_vec) % 7 != 0:
        sys.exit('get_nlyr: model_vec is 0 or not a multiple of 7! Exit.')
...
def calc_resolution_matrices(J=numpy.array([]), G=numpy.array([]),
    ...
    if numpy.size(J) == 0 or numpy.size(G) == 0:
        sys.exit('calc_resolution_matrices: J or G not given. Exit')
...
def calc_model_resolution(J=numpy.array([]), G=numpy.array([]),
    ...
    if numpy.size(J) == 0 or numpy.size(G) == 0:
        sys.exit('calc_model_resolution: J or G not given. Exit')
...
def transform_data(d_vec=numpy.array([]), e_vec=numpy.array([]),
    ...
    if numpy.size(d_vec) == 0:
        sys.exit('transform_data: No data given! Exit.')
...
def transform_parameter(m_vec=numpy.array([]),
    ...
    if m_vec.size == 0:
        sys.exit('transform_para: transform possible, no model given! Exit.')
...
def init_obsdata(nD, value='active'):
    ...
    if value.lower() == 'active':
        ...
        sys.exit('d_act initial  ' + value + ' not implemeted !')
...
def insert_dat(D=numpy.array([]), d=numpy.array([]), d_act=numpy.array([])):
    ...
    '''
    if numpy.size(D) == 0:
        sys.exit('insert_dat: D not defined! Exit.')
    if numpy.size(d) == 0:
        sys.exit('insert_dat: d not defined! Exit.')
    if numpy.size(d_act) == 0:
        sys.exit('insert_dat: d_act not defined! Exit.')
...
def extract_dat(D=numpy.array([]), d_act=numpy.array([])):
    ...
    '''
    if numpy.size(D) == 0:
        sys.exit('extract_dat: D not defined! Exit.')

    if numpy.size(d_act) == 0:
        sys.exit('extract_dat: d_act not defined! Exit.')

    if numpy.size(d_act) != numpy.size(D):
        sys.exit('extract_dat: sizes do not match! Exit.')
...
def insert_jac(M=numpy.array([]),
    ...
    '''
    if numpy.size(M) == 0:
        sys.exit('insert_dat: M not defined! Exit.')

    if m.size == 0:
        sys.exit('insert_dat: m not defined! Exit.')

    if numpy.size(m_act) == 0:
        sys.exit('insert_dat: m_act not defined! Exit.')

    if numpy.size(d_act) == 0:
        sys.exit('insert_jac: d_act not defined! Exit.')
...
def extract_jac(J=numpy.array([]),
    ...
    '''
    if J.size == 0:
        sys.exit('extract_jac: J not defined! Exit.')
    if numpy.size(m_act) == 0:
        sys.exit('extract_jac: m_act not defined! Exit.')
    if numpy.size(d_act) == 0:
        sys.exit('extract_jac: d_act not defined! Exit.')
...
def extract_mod(M=numpy.array([]), m_act=numpy.array([])):
    ...
    '''
    if numpy.size(M) == 0:
        sys.exit('extract_mod: M not defined! Exit.')
    if numpy.size(m_act) == 0:
        sys.exit('extract_mod: m_act not defined! Exit.')
...
def full_cov(C=[numpy.array([])]):
    ...
    if len(C) == 0:
        sys.exit('ful_cov: C not defined! Exit.')
...
def extract_cov(C=numpy.array([]), m_act=numpy.array([])):
    ...
    '''
    if C.size == 0:
        sys.exit('extract_cov: C not defined! Exit.')
    if numpy.size(m_act) == 0:
        sys.exit('extract_cov: m_act not defined! Exit.')
...
def extract_wgt(W=numpy.array([]), m_act=numpy.array([])):
    ...
    '''
    if W.size == 0:
        sys.exit('extract_wgt: W not defined! Exit.')
    if numpy.size(m_act) == 0:
        sys.exit('extract_wgt: m_act not defined! Exit.')
...
def load_prior(prior_file=None,
    ...
    if not os.path.exists(prior_file):
        sys.exit('Prior file '+prior_file+' does not exist! Exit.')
    ...
    if otype == 'L0':
        ...
        if nlyr==1:
            ...
            if variant == 0:
                ...
                sys.exit('DiffOperator variant ' + variant + ' not implemeted ! Exit.')

    else:
        sys.exit('DiffOperator ' + otype + ' not implemeted ! Exit.')
...
def matern(dist=numpy.array([]), var=numpy.array([]), nu=1):
    ...
    if nu <= 0.:
        sys.exit('matern: input argument nu must be positive! Exit.')
    if nu > 100:
        nu = 100.
...
def calc_regstart(D=numpy.array([]), M=numpy.array([]), Fac=1., out=True):
    ...
    if D.size == 0 or M.size == 0:
        sys.exit('cal_regstart: D or M are 0! Exit.')
...
def calc_regstart_base(J=numpy.array([]), W=numpy.array([]), Fac=1., out=True):
    ...
    if J.size == 0 or W.size == 0:
        sys.exit('cal_regstart: J or D are 0! Exit.')
...
def msqrt(M=numpy.array([]), method='cho', smallval=1.e-12):
    '''
    Computes a matrix square-root (cholesky, or eig).
...
def calc_upr(dnorm=numpy.array([]), M=numpy.array([]), err=numpy.array([])):
    ...
    if (numpy.size(dnorm) == 0) or (numpy.size(M) == 0) or (numpy.size(err) == 0):
        sys.exit('calc_upr: parameters missing! Exit.')
...
def calc_mle(M=numpy.array([]),
    ...
    if (numpy.size(M) == 0) or (numpy.size(d) == 0):
        sys.exit('calc_mle: parameters missing! Exit.')
...
def calc_gcv(dnorm=numpy.array([]), M=numpy.array([]), a=1.):
    ...
    if (numpy.size(dnorm) == 0) or (numpy.size(M) == 0):
        sys.exit('calc_gcv: parameters missing! Exit.')
...
def calc_ufc(dnorm=numpy.array([]),
    ...
    '''
    if numpy.size(dnorm) == 0 or numpy.size(mnorm) == 0:
        sys.exit('No model or data norms given! Exit.')
...
def calc_dnorm(data_obs=numpy.array([]),
    ...
    if (numpy.size(data_obs) == 0 or
        ...
        sys.exit('calc_dnorm: parameters missing! Exit.')
...
def calc_datafit(data_obs=numpy.array([]),
    ...
    if (numpy.size(data_obs) == 0 or
        numpy.size(data_cal) == 0 or
            numpy.size(data_err) == 0):
        sys.exit('calc_datafit: parameters missing! Exit.')
...
def set_errors(data_obs=numpy.array([]),
    ...
    if daterr_add == 0. and daterr_mult == 0:
        sys.exit('set_errors: additive and multiplicative error is zero! Exit.')
...
def calc_stat_ens(ensemble=numpy.array([]),
    ...
    '''
    if ensemble.size == 0:
        sys.exit('calc_stat_ens: ensemble not defined')
...
def merge_model_sets(infile_list=None, outfile_name='./mod_tmp.npz',
    ...
    if outfile_name is not None:
        if not '.npz' in os.path.splitext(outfile_name)[1]:
            sys.exit('merge_model_sets: Only npz format implemented.! Exit.')
