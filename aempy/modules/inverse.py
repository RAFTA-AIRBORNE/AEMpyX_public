...
def calc_jac(
    ...
    m_vec:float
        ...
        It is expected in transformed pa
    m_act:int, optional
    ...
    m_trn:int, optional
        ...
        Default is 0.
        0 = only resitivity is transformed to log10
        1 = all parameters are transformed to log10
...
def msqrt(M=numpy.array([]), method="eig"):
    ...
    if "eig" in method.lower():
        ...
        Mevals, Mevecs = scipy.linalg.eigh(M)
...
def msqrt_sparse(A=numpy.array([])):
    """
    ...
    if (LU.perm_r == numpy.arange(n)).all() and (LU.U.diagonal() > 0).all():
        CholA = LU.L.dot(scipy.sparse.diags(LU.U.diagonal() ** 0.5))
        return CholA
    else:
