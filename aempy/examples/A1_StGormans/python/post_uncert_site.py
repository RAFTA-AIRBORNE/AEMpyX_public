...
InModDir =  AEMPYX_DATA+"/home/vrath/Mohammednur/results/"
...
SiteList = True
if SiteList:
    SiteFile = InModDir+"Sites.dat"
    SiteColor = "#0000ff" 
    SiteSize =  2.0
    SiteIcon = "blu-stars.png"
...
for filein in mod_files:
    ...
    if SiteList:
        with open(SiteFile, "w") as file:
            tmp = []
            for ls in numpy.arange(len(site_list)):
                
                lat, lon = util.project_utm_to_latlon(site_x[ls], site_y[ls])
                tmp[0] = lon
                tmp[1] = lat
                
                if "dist" in Sample:
                    tmp[2] = "P: "+str(round(site_r[ls], 0))+" m"
                else:
                    tmp[2] = "P: "+str(site_list[ls])
