...
input format is ".npz"
"""
AEMPYX_DATA =  AEMPYX_ROOT+"/aempy/examples/Example_synth/"
...
if Parallel:
    ...
    joblib.Parallel(n_jobs=Njobs, verbose=100)(
        joblib.delayed(parallel.run_tikh_ensemble)(ctrl=ctrl_dict, data_file=filin,result_strng=outstrng) for filin in dat_files)
else:
...
for file in dat_files:
    ...
    fileout = (OutDatDir + name + OutStrng).replace("_results", "_ctrl_dict")
    numpy.savez_compressed(file=fileout, **ctrl_dict)
    ...
    for ii in samples:
        ...
        if "opt" in RunType.lower():
            results =\
                alg.run_tikh_opt(ctrl_dict=ctrl_dict, Model=Model, Data=Data,
                                  OutInfo=OutInfo)

        if "occ" in RunType.lower():
            results =\
                alg.run_tikh_occ(ctrl_dict=ctrl_dict, Model=Model, Data=Data,
                                  OutInfo=OutInfo)

        if "map" in RunType.lower():
            results =\
                alg.run_map(ctrl_dict=ctrl_dict, Model=Model, Data=Data,
