...
!------------------------------------------------------------------------
! LAST CHANGE:   2 dec  2020   VR
!------------------------------------------------------------------------
!
      !USE core1d_module

      IMPLICIT NONE
      INTEGER , PARAMETER :: nmods=1, nlyr=5, ndata=8
      INTEGER js, mode
      REAL(KIND=8) alt, &
     ...
     &    calc_data(ndata),mvec(7*nlyr),cmp1,cmp2
      ...
      thk(1:nlyr-1)    = (/10.D0, 15.D0, 20.D0, 25.d0 /)
      res(1:nlyr)      = (/100.d0, 10.d0, 100.d0, 1.d0, 100.d0/)
      reps(1:nlyr)     = 1.D0
      rmu(1:nlyr)      = 1.D0
      calf(1:nlyr)     = 0.D0
      ctau(1:nlyr)     = 0.D0
      ...
      CALL cpu_time(cmp1)
      DO js= 1, nmods
             CALL aemfwd1d_aem05( nlyr, mvec, alt,  calc_data)
