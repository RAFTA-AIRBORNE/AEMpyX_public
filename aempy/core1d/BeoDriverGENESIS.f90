...
!------------------------------------------------------------------------
! LAST CHANGE:   26 Sep 2022   VR
!------------------------------------------------------------------------
!
      !USE core1d_module
      IMPLICIT NONE
      INTEGER , PARAMETER :: nmods=1, nlyr=3, ndata = 22
      INTEGER js
      REAL(KIND=8) :: alt, cmp1, cmp2
      REAL(KIND=8), DIMENSION(nlyr) :: thk,res,reps,rmu,calf,ctau,cfreq
      REAL(KIND=8), DIMENSION(ndata) :: calc_data
      REAL(KIND=8), DIMENSION((7*nlyr)) :: mvec
